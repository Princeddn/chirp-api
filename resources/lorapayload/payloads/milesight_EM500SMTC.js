...
function milesightDeviceDecode(bytes) {
    ...
    for (var i = 0; i < bytes.length; ) {
        ...
        // TEMPERATURE
            ...
            var temperature_value = readInt16LE(bytes.slice(i, i + 2));
        ...
        // HISTORY
            ...
            var electricity_value = readUInt16LE(bytes.slice(i + 4, i + 6));
            if (electricity_value === 0xffff) {
                data.electricity_error = readSensorStatus(1);
            } else if (electricity_value === 0xfffd) {
            ...
            if (temperature_value === 0xffff) {
                data.temperature_error = readSensorStatus(1);
            } else if (temperature_value === 0xfffd) {
                data.temperature_error = readSensorStatus(2);
            } else {
                data.temperature = readInt16LE(bytes.slice(i + 6, i + 8)) / 10;
            }
            var moisture_value = readUInt16LE(bytes.slice(i + 8, i + 10));
            if (moisture_value === 0xffff) {
                data.moisture_error = readSensorStatus(1);
            } else if (moisture_value === 0xfffd) {
...
function handle_downlink_response(channel_type, bytes, offset) {
    ...
    switch (channel_type) {
        ...
        case 0x18:
            ...
            if (channel_id === 0x00) {
                decoded.sensor_all_enable = readEnableStatus(bytes[offset + 1]);
            } else if (channel_id === 0x01) {
                decoded.sensor_temperature_enable = readEnableStatus(bytes[offset + 1]);
            } else if (channel_id === 0x02) {
                decoded.sensor_moisture_enable = readEnableStatus(bytes[offset + 1]);
            } else if (channel_id === 0x03) {
                decoded.sensor_electricity_enable = readEnableStatus(bytes[offset + 1]);
            }
            offset += 2;
            break;
        case 0x1c:
...
function readSensorStatus(status) {
    var status_map = { 1: "collection failed", 2: "out of range" };
    return getValue(status_map, status);
}
