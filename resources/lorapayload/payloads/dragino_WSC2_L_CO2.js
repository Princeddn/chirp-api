// https://github.com/dragino/dragino-end-node-decoder/blob/main/WSC2-LB/WSC2-L_co2.txt
function Decode(input) {
  var port = parseInt(input.fPort, 10)    
  var bytes = input.bytes;
	var data = {};
	var decode = {};
	var value;
	var mod=(bytes[31]>>7)&0x01;
	var factor=(bytes[31])&0x0F;	
	var divisor=bytes[32];	 
   ...
   switch (input.fPort) {
    ...
    if(mod===0)
	{
	  ...
	  data.Pressure=((bytes[25]<<8 | bytes[26])/10).toFixed(1);
      data.illumination=(bytes[27]<<24 |bytes[28]<<16 |bytes[29]<<8 | bytes[30]) >>>0;	
	 
	  data.s_flag = (bytes[31]>>6)&0x01;
	  data.i_flag = (bytes[31]>>5)&0x01;
	  data.Mod = mod;
	
	  data.A1 = bytes[34]<<8 | bytes[35];
	  data.A2 = bytes[38]<<8 | bytes[39];
	  data.A3 = bytes[42]<<8 | bytes[43];
	  data.A4 = bytes[46]<<8 | bytes[47];
	}
