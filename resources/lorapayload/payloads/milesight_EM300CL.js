...
function milesightDeviceDecode(bytes) {
    ...
    for (var i = 0; i < bytes.length;) {
        ...
        if (channel_id === 0xff && channel_type === 0x01) {
            decoded.ipso_version = readProtocolVersion(readUInt8(bytes[i]));
            i += 1;
        }
        // HARDWARE VERSION
        ...
        // LORAWAN CLASS TYPE
            ...
            decoded.lorawan_class = readLoRaWANClass(readUInt8(bytes[i]));
            i += 1;
        }
        // RESET EVENT
        ...
        // LIQUID
            ...
            decoded.liquid = readLiquidStatus(readUInt8(bytes[i]));
            i += 1;
        }
        // CALIBRATION RESULT
        else if (channel_id === 0x04 && channel_type === 0xee) {
            decoded.calibration_result = readCalibrationResult(readUInt8(bytes[i]));
            i += 1;
        }
        // LIQUID ALARM
        else if (channel_id === 0x83 && channel_type === 0xed) {
            decoded.liquid = readLiquidStatus(readUInt8(bytes[i]));
            decoded.liquid_alarm = readAlarmType(readUInt8(bytes[i + 1]));
            i += 2;
        }
        // DOWNLINK RESPONSE
