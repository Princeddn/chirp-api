/***
 * https://github.com/dragino/dragino-end-node-decoder/blob/main/WQS-LB/WQS-LB_V1.2_ChirpstackV4_Decoder.txt
 */
...
function decodeUplink(input) {
    ...
    switch (input.fPort) {
        ...
        default:
            ...
            data.i_flag = (bytes[4]>>7)&0x01;
            var sensor_type=(bytes[4])&0x3F;
            var j=5;
            if(((sensor_type>>5)&0x01)==1)
            {
                data.turbidity=(bytes[j]<<8 | bytes[j+1])/10;
                j=2+j;
            }
            if(((sensor_type>>4)&0x01)==1)
            {
                data.dissolved_oxygen=(bytes[j]<<8 | bytes[j+1])/100;
                data.do_temp=(bytes[j+2]<<8 | bytes[j+3])/100;
                j=4+j;
            }
            if(((sensor_type>>3)&0x01)==1)
            {
                data.ORP=(bytes[j]<<24>>16 | bytes[j+1]);
                j=2+j;
            }
            if(((sensor_type>>2)&0x01)==1)
            {
                data.EC_K10=(bytes[j]<<8 | bytes[j+1])*10;
                data.EC10_Temp=(bytes[j+2]<<8 | bytes[j+3])/10;
                j=4+j;
            }
            if(((sensor_type>>1)&0x01)==1)
            {
                data.EC_K1=bytes[j]<<8 | bytes[j+1];
                data.EC1_Temp=(bytes[j+2]<<8 | bytes[j+3])/10;
                j=4+j;
            }
            if(((sensor_type)&0x01)==1)
            {
                data.PH=(bytes[j]<<8 | bytes[j+1])/100;
                data.PH_Temp=(bytes[j+2]<<8 | bytes[j+3])/10;
                j=4+j;
            }
            break;
    }
