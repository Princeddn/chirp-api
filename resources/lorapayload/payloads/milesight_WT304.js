/**
 * 
 * https://github.com/Milesight-IoT/SensorDecoders/tree/main/WT_Series/WT303_WT304
 * Payload Decoder
 *
    ...
    for (var i = 0; i < bytes.length;) {
        ...
        switch (command_id) {
            ...
            case 0xda:
                ...
                decoded.firmware_version = readFirmwareVersion(bytes.slice(i + 2, i + 8));
                i += 8;
                break;
            case 0xd9:
            ...
            case 0x05:
                ...
                if (decoded.temperature_control_mode === 'heating') {
                decoded.heating_target_temperature = decoded.target_temperature;
                } else if (decoded.temperature_control_mode === 'cooling') {
...
function milesightDeviceEncode(payload) {
    ...
    if ("setFourPipeVoltageConfig" in payload) {
        var cmd_buffer = setFourPipeVoltageConfig(payload.setFourPipeVoltageConfig);
        encoded = encoded.concat(cmd_buffer);
        encoded = WITH_QUERY_CMD ? encoded.concat(setQueryCmd(cmd_buffer)) : encoded;
    }

    if ("setTwoPipeVoltageConfig" in payload) {
        var cmd_buffer = setTwoPipeVoltageConfig(payload.setTwoPipeVoltageConfig);
        encoded = encoded.concat(cmd_buffer);
        encoded = WITH_QUERY_CMD ? encoded.concat(setQueryCmd(cmd_buffer)) : encoded;
    }

    if ("setTwoPipeVoltageEcConfig" in payload) {
        var cmd_buffer = setTwoPipeVoltageEcConfig(payload.setTwoPipeVoltageEcConfig);
        encoded = encoded.concat(cmd_buffer);
        encoded = WITH_QUERY_CMD ? encoded.concat(setQueryCmd(cmd_buffer)) : encoded;
    }

    if ("setFourPipeTwoWireEcConfig" in payload) {
        var cmd_buffer = setFourPipeTwoWireEcConfig(payload.setFourPipeTwoWireEcConfig);
        encoded = encoded.concat(cmd_buffer);
        encoded = WITH_QUERY_CMD ? encoded.concat(setQueryCmd(cmd_buffer)) : encoded;
    }

    if ("setTwoPipeTwoWireEcConfig" in payload) {
        var cmd_buffer = setTwoPipeTwoWireEcConfig(payload.setTwoPipeTwoWireEcConfig);
        encoded = encoded.concat(cmd_buffer);
        encoded = WITH_QUERY_CMD ? encoded.concat(setQueryCmd(cmd_buffer)) : encoded;
    }

    if ("setTwoPipeThreeWireECConfig" in payload) {
        var cmd_buffer = setTwoPipeThreeWireECConfig(payload.setTwoPipeThreeWireECConfig);
        encoded = encoded.concat(cmd_buffer);
        encoded = WITH_QUERY_CMD ? encoded.concat(setQueryCmd(cmd_buffer)) : encoded;
    }
    return encoded;
...
function setChildLockSettings(child_lock_settings) {
  // --- 1) Normalisation en objet ---
  let obj;
  if (typeof child_lock_settings === 'string') {
    const p = child_lock_settings.split(/[|,]/).map(s => s.trim());
    if (p.length !== 6) {
      throw new Error("child_lock_settings string must be 'enable|system|temperature|fan|temperature_control|reboot'");
    }
    obj = {
      enable: p[0],
      system_button: p[1],
      temperature_button: p[2],
      fan_button: p[3],
      temperature_control_button: p[4],
      reboot_reset_button: p[5]
    };
  } else if (Array.isArray(child_lock_settings)) {
    if (child_lock_settings.length !== 6) {
      throw new Error("child_lock_settings array must have 6 items: [enable,system,temperature,fan,temperature_control,reboot]");
    }
    const [e, s, t, f, tc, r] = child_lock_settings;
    obj = {
      enable: e,
      system_button: s,
      temperature_button: t,
      fan_button: f,
      temperature_control_button: tc,
      reboot_reset_button: r
    };
  } else if (child_lock_settings && typeof child_lock_settings === 'object') {
    obj = child_lock_settings;
  } else {
    throw new Error("child_lock_settings must be object, string or array");
  }

  // --- 2) Helper pour convertir vers 0/1 ---
  const to01 = (v) => {
    if (typeof v === 'string') v = v.toLowerCase();
    if (v === 1 || v === '1' || v === true || v === 'enable' || v === 'enabled' || v === 'on') return 1;
    if (v === 0 || v === '0' || v === false || v === 'disable' || v === 'disabled' || v === 'off') return 0;
    const n = parseInt(v, 10);
    if (!isNaN(n)) return n ? 1 : 0;
    throw new Error("Invalid child lock value: " + v);
  };

  // --- 3) Construction des octets ---
  const enableByte = to01(obj.enable);

  const bitOffset = {
    system_button: 0,
    temperature_button: 1,
    fan_button: 2,
    temperature_control_button: 3,
    reboot_reset_button: 4
  };
  let bitmap = 0;
  Object.keys(bitOffset).forEach(k => {
    if (obj[k] !== undefined && to01(obj[k]) === 1) {
      bitmap |= (1 << bitOffset[k]);
    }
  });

  // --- 4) Buffer résultat : 75 | enable | bitmap ---
  var buffer = new Buffer(3);
  buffer.writeUInt8(0x75);
  buffer.writeUInt8(enableByte);
...
/** Set Four Pipe Voltage Config
 * @param {object} setFourPipeVoltageConfig
 * @param {number} setFourPipeVoltageConfig.cooling_valve values: (0: valve_1, 1: valve_2)
 * @param {number} setFourPipeVoltageConfig.heating_valve values: (0: valve_1, 1: valve_2)
 * @example { "setFourPipeVoltageConfig": { "cooling_valve": 0, "heating_valve": 1 } }
 */
function setFourPipeVoltageConfig(setFourPipeVoltageConfig) { // OK 7C 00 00 01
    var cooling_valve = setFourPipeVoltageConfig.cooling_valve;
    var heating_valve = setFourPipeVoltageConfig.heating_valve;

    // mapping correct : labels -> codes numériques
    var valve_map = { "valve_1": 0, "valve_2": 1 };

    // normalisation (si tu reçois 0 ou 1 directement, on les mappe aussi)
    if (cooling_valve === 0 || cooling_valve === "0") cooling_valve = "valve_1";
    if (cooling_valve === 1 || cooling_valve === "1") cooling_valve = "valve_2";
    if (heating_valve === 0 || heating_valve === "0") heating_valve = "valve_1";
    if (heating_valve === 1 || heating_valve === "1") heating_valve = "valve_2";

    if (!(cooling_valve in valve_map)) {
        throw new Error("setFourPipeVoltageConfig.cooling_valve must be 0/1 or valve_1/valve_2");
    }
    if (!(heating_valve in valve_map)) {
        throw new Error("setFourPipeVoltageConfig.heating_valve must be 0/1 or valve_1/valve_2");
    }

    var buffer = new Buffer(4);
    buffer.writeUInt8(0x7c);    // Command class
    buffer.writeUInt8(0x00);    // Sub-command for four_pipe_voltage
    buffer.writeUInt8(valve_map[cooling_valve]);
    buffer.writeUInt8(valve_map[heating_valve]);
    console.log(buffer.toBytes());
    return buffer.toBytes();
}

/** Set Two Pipe Voltage Config
 * @param {object} setTwoPipeVoltageConfig
 * @param {number} setTwoPipeVoltageConfig.valve values: (0: valve_1, 1: valve_2)
 * @example { "setTwoPipeVoltageConfig": { "control_valve": 1 } }
 */
function setTwoPipeVoltageConfig(config) { // OK 7C 01 01
    var control_valve = config.valve;
    console.log("setTwoPipeVoltageConfig.control_valve=", control_valve);

    var valve_map = { "valve_1": 0x01, "valve_2": 0x02 };

    if (!(control_valve in valve_map)) {
        throw new Error("two_pipe_voltage.control_valve must be one of " + Object.keys(valve_map).join(", "));
    }

    var buffer = new Buffer(3);
    buffer.writeUInt8(0x7c);
    buffer.writeUInt8(0x01); // sub-command
    buffer.writeUInt8(valve_map[control_valve]);
    return buffer.toBytes();
}


/** Set Two Pipe Voltage EC Config
 * @param {object} setTwoPipeVoltageEcConfig
 * @param {number} setTwoPipeVoltageEcConfig.valve values: (0: valve_1, 1: valve_2)
 * @param {number} setTwoPipeVoltageEcConfig.ec values: (0: valve_1, 1: valve_2)
 * @param {number} setTwoPipeVoltageEcConfig.ec_power values: (0: none, 3: low, 4: medium, 5: high)
 * @example { "setTwoPipeVoltageEcConfig": { "control_valve": 0, "ec": 1, "ec_power": 4 } }
 */
function setTwoPipeVoltageEcConfig(config) { // OK 7C 02 01 00 04
    var control_valve = config.valve;
    var ec = config.ec;
    var ec_power = config.ec_power;

    // mapping : nom logique → code binaire
    var valve_map = { "valve_1": 0x00, "valve_2": 0x01 };
    var ec_map = { "valve_1": 0x00, "valve_2": 0x01 };
    var ec_power_map = { "none": 0x00, "low": 0x03, "medium": 0x04, "high": 0x05 };

    // validation
    if (!(control_valve in valve_map)) {
        throw new Error("two_pipe_voltage_ec.control_valve must be one of " + Object.keys(valve_map).join(", "));
    }
    if (!(ec in ec_map)) {
        throw new Error("two_pipe_voltage_ec.ec must be one of " + Object.keys(ec_map).join(", "));
    }
    if (!(ec_power in ec_power_map)) {
        throw new Error("two_pipe_voltage_ec.ec_power must be one of " + Object.keys(ec_power_map).join(", "));
    }
    ...
    buffer.writeUInt8(valve_map[control_valve]);
    buffer.writeUInt8(ec_map[ec]);
    buffer.writeUInt8(ec_power_map[ec_power]);
    return buffer.toBytes();
}


/** Set Four Pipe Two Wire EC Config
 * @param {object} setFourPipeTwoWireEcConfig
 * @param {number} setFourPipeTwoWireEcConfig.cooling_valve values: (3: low, 4: medium, 5: high)
 * @param {number} setFourPipeTwoWireEcConfig.heating_valve values: (3: low, 4: medium, 5: high)
 * @param {number} setFourPipeTwoWireEcConfig.ec values: (0: valve_1, 1: valve_2)
 * @param {number} setFourPipeTwoWireEcConfig.ec_power values: (0: none, 3: low, 4: medium, 5: high)
 * @example { "setFourPipeTwoWireEcConfig": { "cooling_valve": 3, "heating_valve": 4, "ec": 0, "ec_power": 5 } }
 */
function setFourPipeTwoWireEcConfig(config) { // OK
    var cooling_valve = config.cooling_valve;
    var heating_valve = config.heating_valve;
    var ec = config.ec;
    var ec_power = config.ec_power;

    // mapping direct des valeurs numériques
    var mode_map = { "low": 0x03, "medium": 0x04, "high": 0x05 }; // low, medium, high
    var valve_map = { "valve_1": 0x00, "valve_2": 0x01 };         // valve_1, valve_2
    var ec_power_map = { "none": 0x00, "low": 0x03, "medium": 0x04, "high": 0x05 }; // none, low, medium, high

    // validation
    if (!(cooling_valve in mode_map)) {
        throw new Error("four_pipe_two_wire_ec.cooling_valve must be one of " + Object.keys(mode_map).join(", "));
    }
    if (!(heating_valve in mode_map)) {
        throw new Error("four_pipe_two_wire_ec.heating_valve must be one of " + Object.keys(mode_map).join(", "));
    }
    if (!(ec in valve_map)) {
        throw new Error("four_pipe_two_wire_ec.ec must be one of " + Object.keys(valve_map).join(", "));
    }
    if (!(ec_power in ec_power_map)) {
        throw new Error("four_pipe_two_wire_ec.ec_power must be one of " + Object.keys(ec_power_map).join(", "));
    }
    ...
    buffer.writeUInt8(mode_map[cooling_valve]);
    buffer.writeUInt8(mode_map[heating_valve]);
    buffer.writeUInt8(valve_map[ec]);
    buffer.writeUInt8(ec_power_map[ec_power]);
    return buffer.toBytes();
}

/** Set Two Pipe Two Wire EC Config
 * @param {object} setTwoPipeTwoWireEcConfig
 * @param {number} setTwoPipeTwoWireEcConfig.control_valve values: (3: low, 4: medium, 5: high)
 * @param {number} setTwoPipeTwoWireEcConfig.ec values: (0: valve_1, 1: valve_2)
 * @param {number} setTwoPipeTwoWireEcConfig.ec_power values: (0: low, 3: medium, 4: high, 5: none)
 * @example { "setTwoPipeTwoWireEcConfig": { "control_valve": 4, "ec": 1, "ec_power": 3 } }
 */
function setTwoPipeTwoWireEcConfig(config) { // OK
    var control_valve = config.control_valve;
    var ec = config.ec;
    var ec_power = config.ec_power;

    // mappings
    var control_valve_map = { "low": 0x03, "medium": 0x04, "high": 0x05 };
    var ec_map = { "valve_1": 0x00, "valve_2": 0x01 };
    var ec_power_map = { "none": 0x05, "low": 0x00, "medium": 0x03, "high": 0x04 };

    // validation
    if (!(control_valve in control_valve_map)) {
        throw new Error("two_pipe_two_wire_ec.control_valve must be one of " + Object.keys(control_valve_map).join(", "));
    }
    if (!(ec in ec_map)) {
        throw new Error("two_pipe_two_wire_ec.ec must be one of " + Object.keys(ec_map).join(", "));
    }
    if (!(ec_power in ec_power_map)) {
        throw new Error("two_pipe_two_wire_ec.ec_power must be one of " + Object.keys(ec_power_map).join(", "));
    }

    // construction du buffer
    var buffer = new Buffer(5); // 1 byte de plus que ton code
    buffer.writeUInt8(0x7c);
    buffer.writeUInt8(0x04); // two_pipe_two_wire_ec sub-command
    buffer.writeUInt8(control_valve_map[control_valve]);
    buffer.writeUInt8(ec_map[ec]);
    buffer.writeUInt8(ec_power_map[ec_power]);
    return buffer.toBytes();
}

/** Set Two Pipe Three Wire EC Config
 * @param {object} setTwoPipeThreeWireECConfig
 * @param {number} setTwoPipeThreeWireECConfig.no values: (3: low, 4: medium, 5: high)
 * @param {number} setTwoPipeThreeWireECConfig.nc values: (3: low, 4: medium, 5: high)
 * @param {number} setTwoPipeThreeWireECConfig.ec values: (0: valve_1, 1: valve_2)
 * @param {number} setTwoPipeThreeWireECConfig.ec_power values: (0: low, 3: medium, 4: high, 5: none)
 * @example { "setTwoPipeThreeWireECConfig": { "no": 3, "nc": 5, "ec": 1, "ec_power": 4 } }
 */
function setTwoPipeThreeWireECConfig(config) { // OK
    var no = config.no;
    var nc = config.nc;
    var ec = config.ec;
    var ec_power = config.ec_power;
    ...
    // mappings
    var n_map = { "low": 0x03, "medium": 0x04, "high": 0x05 };
    var ec_map = { "valve_1": 0x00, "valve_2": 0x01 };
    var ec_power_map = { "none": 0x05, "low": 0x00, "medium": 0x03, "high": 0x04 };

    // validation
    if (!(no in n_map)) {
        throw new Error("two_pipe_three_wire_ec.no must be one of " + Object.keys(n_map).join(", "));
    }
    if (!(nc in n_map)) {
        throw new Error("two_pipe_three_wire_ec.nc must be one of " + Object.keys(n_map).join(", "));
    }
    if (!(ec in ec_map)) {
        throw new Error("two_pipe_three_wire_ec.ec must be one of " + Object.keys(ec_map).join(", "));
    }
    if (!(ec_power in ec_power_map)) {
        throw new Error("two_pipe_three_wire_ec.ec_power must be one of " + Object.keys(ec_power_map).join(", "));
    }
    ...
    buffer.writeUInt8(n_map[no]);
    buffer.writeUInt8(n_map[nc]);
    buffer.writeUInt8(ec_map[ec]);
    buffer.writeUInt8(ec_power_map[ec_power]);
    return buffer.toBytes();
}
...
function setQueryCmd(bytes) {
    var name_map = {
        ...
        "7a": { "level": 1, "name": "humidity_calibration_config" },
        "7b": { "level": 3, "name": "plan_config" },
        "7c00": { "level": 1, "name": "setFourPipeVoltageConfig" },
        "7c01": { "level": 1, "name": "two_pipe_voltage" },
        "7c02": { "level": 1, "name": "two_pipe_voltage_ec" },
        "7c03": { "level": 1, "name": "four_pipe_two_wire_ec" },
        "7c04": { "level": 1, "name": "two_pipe_two_wire_ec" },
        "7c05": { "level": 1, "name": "two_pipe_three_wire_ec" },
        "7d": { "level": 1, "name": "valve_control_settings" },
        "7e": { "level": 1, "name": "fan_control_settings" },
        "80": { "level": 1, "name": "di_setting_enable" },
        "81": { "level": 1, "name": "di_settings" },
...
function getValue(map, value) {
    return map.hasOwnProperty(value) ? map[value] : "unknown";
}
