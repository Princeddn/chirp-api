...
//https://github.com/browanofficial/Sensor-Decoder/blob/main/tabs_sensor/Decoder_tabssensor_forTTN_v4.1.txt
...
// tabs sensor- Temperature & Humidity Sensor
function DecodeOLD(input) {
  var fPort = parseInt(input.fPort);
  var payloadlens = input.bytes.length;
  console.log("Payload Lens : ", payloadlens)
  const decoded = {};
  fPort = 103;
  ...
  if(fPort==222 && payloadlens==17){//Frame Count 0 Content
    ...
    var fwcrc_hex = intput_liststr.toString("hex").substring(18, 26);
    bootloader = littleendianhex(bootloader_hex);
    decoded.bootloader = bootloader;
    HW_ID = littleendianhex(hwid_hex);
    decoded.HW_ID = HW_ID;
    FW_CRC = littleendianhex(fwcrc_hex);
    decoded.FW_CRC = FW_CRC;
    ...
    var battery_int = parseInt(battery_binary2,2)
    battery_volt = (25+battery_int)/10; // battery voltage
    decoded.battery_volt = battery_volt;
    ...
    decoded.temperature_pcb = temperature_pcb;
    ...
    decoded.temperature_env = temperature_env;
