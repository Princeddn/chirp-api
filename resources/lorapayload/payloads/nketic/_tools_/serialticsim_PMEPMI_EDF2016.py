...
#- DATEPA1 varie de 10min en 10min toutes les DATEPA1_DT(60) :  DATE date courante , et +1 sur  PA1_s, EAP_s, EAP_i, ER+P_s, ER-P_s, 
#- PTCOUR1 varie entre HPH, HCH, HCE, HPE toutes les PTCOUR1_DT(120) : et +1 sur  EaP-1_s, ER+P-1_s, PS
#- PTCOUR2 varie entre HPH, HCH, HCE, HPE toutes les PTCOUR2_DT(180) : et +1 sur  EaP-1_s2
...
PTCOUR1_DT = 60 #----------- Specifer la periode
...
PTCOUR2_DT = 60 #----------- Specifer la periode 
EaPmoins1_s2 = 0

TARIFDYN_DT = 60 #----------- Specifer la periode 
TDYN1FD = 0
TDYN1FF = 0
TDYN2FD = 0
TDYN2FF = 0

PREAVIS_DT = 360 #----------- Specifer la periode
# PREAVIS_DU = 10
on_preavis = False

PS=80 # Set PS to 80kVA
EAPP_s = 0.0
...
while True:
	...
	for line in array:
		...
		if(line == '>'):
			...
			if (firstWord == 'DATE'):
			...
			elif (firstWord == 'PS'): # Set required PS
				line = "PS %dkVA" % (PS)
				line = line + " %c" % edf_checksum(line)
				
			elif (firstWord == 'EAPP_s'): # Set required EAPP_s
				if ((idx_PTCOUR1 == 0) or (idx_PTCOUR1 == 2)) : # Depassement uniquement en HPH et HPE
					# Increase EAPP_s during PTCOUR duration with a "Depassement de PS"
					# Notice TJ => using EAPP_s
					# Energie a consommer entre 2 trames pour etre en depassement (ici 2 x c'est enorme !!)
					EAPP_s += (2 * PS * 1000.0) * ((theNow - theNowPrec).seconds / 3600.0)
				else :
					# No depassement during other periods (1/10 de PS)
					EAPP_s += ((0.1 * PS * 1000.0)*((theNow - theNowPrec).seconds / 3600.0))
				line = "EAPP_s %dVAh" % (EAPP_s)
				line = line + " %c" % edf_checksum(line)
				...
				if(((theNow -lastDatePTCOUR1).seconds) >= PTCOUR1_DT ):
					...
					# back EAPP_s to 0 when PT_COUR1 changing (used in "Test de depassement")
					EAPP_s = 0
