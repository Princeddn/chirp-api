 ...
 *
 * https://github.com/Milesight-IoT/SensorDecoders/tree/main/vs-series/vs350
 * 
 * @product VS350
 */
var RAW_VALUE = 0x00;
...
function handle_downlink_response(channel_type, bytes, offset) {
    ...
    switch (channel_type) {
        case 0x06:
            ...
            decoded = {};
            if (trigger_source === 1) {
                decoded.condition_ppac = readConditionType(data & 0x07);
                decoded.threshold_out_ppac = readUInt16LE(bytes.slice(offset + 1, offset + 3));
                decoded.threshold_in_ppac = readUInt16LE(bytes.slice(offset + 3, offset + 5));
            } else if (trigger_source === 2) {
                decoded.condition_pcac = readConditionType(data & 0x07);
                decoded.threshold_out_pcac = readUInt16LE(bytes.slice(offset + 1, offset + 3));
                decoded.threshold_in_pcac = readUInt16LE(bytes.slice(offset + 3, offset + 5));
            } else if (trigger_source === 3) {
                decoded.condition_tac = readConditionType(data & 0x07);
                decoded.threshold_min_tac = readUInt16LE(bytes.slice(offset + 1, offset + 3)) / 10;
                decoded.threshold_max_tac = readUInt16LE(bytes.slice(offset + 3, offset + 5)) / 10;
            }
            offset += 9;
            break;
        case 0x10:
        ...
        case 0xaa:
            ...
            break;
        case 0xab:
            decoded.temperature_calibration_enable = readEnableStatus(bytes[offset]);
            decoded.temperature_calibration_value = readInt16LE(bytes.slice(offset + 1, offset + 3)) / 10;
            offset += 3;
            break;
        case 0xbd:
...
function milesightDeviceEncode(payload) {
    ...
    for (var key in payload) {
        if (payload.hasOwnProperty(key) && typeof payload[key] === 'string') {
            // Parse people alarm configs: condition|threshold_in|threshold_out
            if (key === 'people_period_alarm_config' || key === 'people_cumulative_alarm_config') {
                var parts = payload[key].split('|');
                if (parts.length >= 3) {
                    payload[key] = {
                        condition: parts[0],
                        threshold_in: parseInt(parts[1]),
                        threshold_out: parseInt(parts[2])
                    };
                }
            }
            // Parse temperature alarm config: condition|threshold_min|threshold_max
            else if (key === 'temperature_alarm_config') {
                var parts = payload[key].split('|');
                if (parts.length >= 3) {
                    payload[key] = {
                        condition: parts[0],
                        threshold_min: parseFloat(parts[1]),
                        threshold_max: parseFloat(parts[2])
                    };
                }
            }
            // Parse temperature calibration: enable|calibration_value
            else if (key === 'temperature_calibration_settings') {
                var parts = payload[key].split('|');
                if (parts.length >= 2) {
                    payload[key] = {
                        enable: parts[0],
                        calibration_value: parseFloat(parts[1])
                    };
                }
            }
            // For d2d_master_config, try JSON
            else if (key === 'd2d_master_config') {
                try {
