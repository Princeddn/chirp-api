...
function milesightDeviceDecode(bytes) {
    ...
    for (var i = 0; i < bytes.length; ) {
        ...
        // HISTORICAL DATA
            ...
            var mode = bytes[i + 7];
            if (mode === 1) {
                decoded.gpio_type = "gpio";
                decoded.gpio = bytes[i + 8];
            } else if (mode === 2) {
                decoded.gpio_type = "pulse";
                decoded.pulse = readUInt32LE(bytes.slice(i + 9, i + 13));
            }
        ...
        // HISTORICAL DATA (v2)
            ...
            if (mode === 1) {
                decoded.gpio_type = "gpio";
                decoded.gpio = readGPIOStatus(bytes[i + 9]);
            } else if (mode === 2) {
                decoded.gpio_type = "pulse";
                decoded.water_conv = readUInt16LE(bytes.slice(i + 10, i + 12)) / 10;
                decoded.pulse_conv = readUInt16LE(bytes.slice(i + 12, i + 14)) / 10;
                decoded.water = readFloatLE(bytes.slice(i + 14, i + 18));
            }
