/**
 * 
 * https://github.com/Milesight-IoT/SensorDecoders/blob/main/WT_Series/WT101/WT101_Decoder.js
 * Payload Decoder
 *
...
function milesightDeviceDecode(bytes) {
    ...
    for (var i = 0; i < bytes.length; ) {
        ...
        // INSTALLATION STATUS
            ...
            decoded.tamper_status = bytes[i] === 0 ? 1 : 0;
            i += 1;
        }
        // FEENSTRATION STATUS
        else if (channel_id === 0x07 && channel_type === 0x00) {
            decoded.window_detection = bytes[i] === 0 ? 0 : 1;
            i += 1;
        }
        ...
        // FROST PROTECTION
            ...
            decoded.freeze_protection = bytes[i] === 0 ? 0 : 1;
            i += 1;
        }
 ...
 * @since v1.3
 * @param {object|number} effective_stroke - If number: 0=disable, 1-100=enable with rate. If object, must contain enable and rate properties.
 ...
 * @example { "effective_stroke": 50 } // Shorthand for { "enable": 1, "rate": 50 }
 * @example { "effective_stroke": 0 } // Shorthand for { "enable": 0, "rate": 0 } (disable)
 */
function setEffectiveStroke(effective_stroke) {
    var enable, rate;

    // Si effective_stroke est un nombre
    if (typeof effective_stroke === "number") {
        if (effective_stroke === 0) {
            // 0 signifie disable
            enable = 0;
            rate = 0;
        } else {
            // 1-100 signifie enable avec le rate
            enable = 1;
            rate = effective_stroke;
        }
    } else {
        enable = effective_stroke.enable;
        rate = effective_stroke.rate;
    }

    var enable_map = { 0: "disable", 1: "enable" };
    // Vérifier si enable est une clé valide (0 ou 1)
    if (!(enable in enable_map)) {
        throw new Error("effective_stroke.enable must be 0 or 1");
    }
