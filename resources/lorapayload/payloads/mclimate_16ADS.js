/* https://docs.mclimate.eu/mclimate-lorawan-devices/devices/mclimate-16a-dry-switch-16ads/mclimate-16ads-uplink-decoder */
const Decode = (input) => {
    try {
        let { bytes } = input;
        let data = {};
        ...
        const handleKeepalive = (bytes, data) => {
            ...
            // Relay state
            data.relayState = bytes[2] === 0x01 ? "1" : "0";

            return data;
        };

        const handleResponse = (bytes, data) => {
            let commands = bytes.map(byte => (`0${byte.toString(16)}`).slice(-2)).slice(0, -2);
            let command_len = 0;
            ...
            commands.forEach((command, i) => {
                switch (command) {
                    ...
                    case '1f': {
                        ...
                        data.trigger = parseInt(commands[i + 1], 16);
                        data.recovery = parseInt(commands[i + 2], 16);
                        break;
                    }
                    case '5a': {
                        command_len = 1;
                    ...
                    case '60': {
                        ...
                        data.events = parseInt(commands[i + 1], 16);
                        data.temperature = parseInt(commands[i + 2], 16);
                        break;
                    }
                    case '70': {
                        command_len = 2;
                    ...
                    case 'a0': {
                        ...
                        data.fuota_address = fuota_address;
                        data.fuota_address_raw = fuota_address_raw;
                        break;
                    }
                    case 'a4': {
                        command_len = 1;
