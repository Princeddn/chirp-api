//https://github.com/dragino/dragino-end-node-decoder/blob/main/WSC2-LB/WSC2-L_pm2.5.txt
function Decode(input) {
  ...
  console.log("valeur du port: ",port )
  if (port == 1) {
    port = 2;
  }
...
switch (port) {
	...
	// Temperature avec gestion des valeurs négatives (signed 16-bit)
	var tempRaw = bytes[19]<<8 | bytes[20];
	if(bytes[19] & 0x80) {
		tempRaw = tempRaw - 0x10000; // Convert to signed
	}
	data.Temperature=(tempRaw/10).toFixed(1);
