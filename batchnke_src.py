...
def main():
    ...
    if args.timestamp:
        ...
        print(json.dumps(uncompress(args.tagsz, commands, new_input, args.timestamp), indent=4))
    else:
...
def uncompress(tagsz, arg_list, hex_string, batch_absolute_timestamp=None):
    ...
    for i in range(flag.nb_of_type_measure):
        tag = Tag(size=tagsz, lbl=data_buffer.next_sample(constants.ST_U8, tagsz))
        ...
        P.print(f" Measure {out.series[ii].uncompress_samples[0].data.value}", end="")

        if not flag.no_sample:
            out.series[ii].coding_type = data_buffer.next_sample(constants.ST_U8, 2)
            out.series[ii].coding_table = data_buffer.next_sample(constants.ST_U8, 2)
            P.print(
    ...
    if not flag.no_sample:
        if flag.cts:
            ...
            for j in range(flag.nb_of_type_measure):
                ...
                tag = Tag(
                    size=tagsz, lbl=data_buffer.next_sample(constants.ST_U8, tagsz)
                )
                P.print("")
                ...
                for i in range(0, nb_sample_to_parse):
                    ...
                    if available:
                        P.print(f"coding table: {out.series[ii].coding_table}", end="")
                        # Delta value
            ...
            for i in range(flag.nb_of_type_measure):
                ...
                ii = find_index_of_lbl(arg_list, tag.lbl)
                compress_samples_nb = data_buffer.next_sample(constants.ST_U8, 8)
                P.print(
                    ...
                    for j in range(compress_samples_nb):
                        ...
                        if bi <= constants.BR_HUFF_MAX_INDEX_TABLE:
                            if bi > 0:
                                ...
                                P.print(f"  Value: {current_measure.data.value}")
                            else:  # bi <= 0
                                current_measure.data.value = (
                                    out.series[ii].uncompress_samples[-1].data.value
                                )
    ...
    if not last_timestamp:
        ...
        if bi <= constants.BR_HUFF_MAX_INDEX_TABLE:
            if bi > 0:
                global_timestamp = data_buffer.next_sample(constants.ST_U32, bi)
